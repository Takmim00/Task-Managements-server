...
async function run() {
  try {
    ...
    const messagesCollection = db.collection("messages"); // New Collection for Messages
    ...
    io.on("connection", (socket) => {
      ...
      console.log(insertedTask);
        socket.to(category).emit("receive_message", insertedTask);
      });
    ...
    app.put("/tasks/:id", async (req, res) => {
      ...
      try {
        ...
        if (result.modifiedCount > 0) {
          ...
          res.status(404).send({ message: "Message not found or no changes made." });
        }
      } catch (error) {
